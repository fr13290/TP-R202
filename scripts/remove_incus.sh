#!/bin/bash
################################################################################
# DÉSINSTALLATION COMPLÈTE D'INCUS
...
echo -e "${BLUE}[1/6]${NC} Arrêt du service Incus..."
...
echo -e "${BLUE}[2/6]${NC} Suppression des conteneurs et VMs..."
INSTANCE_COUNT=0
...
echo -e "${BLUE}[3/6]${NC} Suppression des réseaux personnalisés..."
NETWORK_COUNT=0
...
echo -e "${BLUE}[4/6]${NC} Suppression du réseau incusbr0..."
incus network delete incusbr0 2>/dev/null || true
echo -e "${GREEN}  ✓${NC} incusbr0 supprimé"

# 5. Désinstaller les paquets Incus
echo -e "${BLUE}[5/6]${NC} Désinstallation des paquets Incus..."
apt-get purge -y incus incus-client incus-extra incus-base 2>/dev/null || true
apt-get autoremove -y 2>/dev/null || true
apt-get autoclean 2>/dev/null || true
echo -e "${GREEN}  ✓${NC} Paquets supprimés"

# 6. Supprimer toutes les données Incus
echo -e "${BLUE}[6/6]${NC} Suppression des données Incus..."
rm -rf /var/lib/incus
rm -rf /var/log/incus
